...
def rainbow(strip, wait_ms, iterations=1):
    """Draw rainbow effect across all pixels with adjustable speed."""
    for j in range(256 * iterations):
        for i in range(strip.numPixels()):
            strip.setPixelColor(i, wheel((i + j) & 255))
        strip.show()
        time.sleep(wait_ms / 1000.0)  # User-defined delay in milliseconds
...
def start_animation():
    ...
    delay = int(request.args.get('delay', 20))  # Default delay = 20ms
