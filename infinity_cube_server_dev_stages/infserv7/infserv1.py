...
def set_pixels(pixel_data):
    print(f"Received data: {repr(pixel_data)}")
    
    try:
        values = list(map(int, pixel_data.strip().split(',')))
    except ValueError:
        print("Error: Cannot convert values to integers!")
        return False
    
    if len(values) % 3 != 0:
        print(f"Invalid number of values ({len(values)})! Must be a multiple of 3.")
        return False
